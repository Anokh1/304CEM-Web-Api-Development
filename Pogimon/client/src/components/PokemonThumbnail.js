...
const PokemonThumbnail = ({id, name, image, type}) => {
  ...
  async function pokeClickedFavorite(event) {
    ...
    if (data.status === 'ok'){
      console.log('Favourite Pokemon button ok') // data sent to 'favourite-monster' collection
    }
  }
  ...
  return (
    <div className={style}>
      <div className='shiny-button'>
        {/* <button>SHINY</button> */}
        <span></span>
        <button className='favourite-Pokemon' onClick={pokeClickedFavorite}>❤</button>
      </div>
